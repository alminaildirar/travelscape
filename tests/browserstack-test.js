...
async function runTestOnBrowserStack(capability) {
  ...
  try {
    ...
    await driver.executeScript(
      'browserstack_executor: {"action": "setSessionStatus", "arguments": {"status":"passed","reason": "All tests passed"}}',
    );

    console.log(`\n✅ ${browserName} - All tests passed!\n`);
    return true;
  } catch (error) {
    console.error(`\n❌ ${browserName} - Test failed:`, error.message, '\n');
...
async function runAllBrowserStackTests() {
  ...
  console.log('='.repeat(70));
  console.log(
    `📋 Testing ${config.capabilities.length} browser/device configurations`,
  );
